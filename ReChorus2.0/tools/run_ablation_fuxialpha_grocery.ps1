...
Write-Host "[INFO] dataset=$dataset appendix=$DataAppendix epoch=$Epoch early_stop=$EarlyStop history_max=$HistoryMax lr=$FuXiLr attn_dropout=$FuXiAttnDropout"
...
function Run-One {
  ...
  $args = @(
    ...
    "--ff_hidden_size", "256",
    "--attn_dropout", ([string]$FuXiAttnDropout),
    "--fuxi_use_pos", "$UsePos",
...
Write-Host "\n[DONE] $datasetName ablation finished." -ForegroundColor Green

...
> 课程作业仓库说明（重要）
>
> 为了保证本仓库可以在 GitHub 上正常推送（体积限制）并尊重数据集的分发/版权要求，
> 本仓库 **不会上传任何原始或预处理后的数据文件**。
>
> 你需要按本 README 的说明自行下载/预处理数据，并放到本目录下对应的子文件夹中。
> 目录名应保持为：`MovieLens_1M/`、`MovieLens_100K/`、`Grocery_and_Gourmet_Food/`、`MIND_Large/`。

...
def parse_global_args(parser):
	...
	parser.add_argument('--data_appendix', type=str, default='',
						help='Appendix string for dataset/log/model naming (e.g., _sweep1).')
...
def _safe_basename(name: str, max_len: int = 180) -> str:
	"""Make a filesystem-safe, reasonably short basename.

	Windows can error on very long file paths. ReChorus log/model filenames may become
	very long when many hyperparameters are embedded in the name. This helper:
	- replaces characters invalid on Windows
	- truncates and appends a stable hash for uniqueness
	"""
	...
	keep = max(40, max_len - (2 + len(h)))
	return name[:keep] + "__" + h
...
if __name__ == '__main__':
	...
	# Respect user-provided --data_appendix; only auto-fill when needed.
	if not hasattr(args, 'data_appendix') or args.data_appendix is None:
		args.data_appendix = ''
	if 'Context' in model_name.reader:
		ctx_appendix = '_context%d%d%d'%(args.include_item_features,args.include_user_features,
								args.include_situation_features)
		if args.data_appendix == '':
			args.data_appendix = ctx_appendix
		else:
			# keep both if user already set one
			args.data_appendix = str(args.data_appendix) + ctx_appendix
	...
	log_file_name = _safe_basename(log_file_name)
	if args.log_file == '':
